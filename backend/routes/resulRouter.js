...
router.get('/cities/circles/voted-count', async (req, res) => {
  ...
  await Promise.all(finalResults.map(async entry => {
    await knex('candidates')
      .whereIn('list', entry.passingCandidates.map(candidate => candidate.list))
    ...
    await Promise.all(finalResults.map(async entry => {
        await Promise.all(entry.passingCandidates.map(async candidate => {
          await knex('candidates')
          .whereIn('list', entry.passingCandidates.map(candidate => candidate.list))
...
router.get('/candidates/passedf', async (req, res) => {
    try {
      ...
      const passedCandidates = await knex('candidates')
        ...
        .orderBy(['city', 'circle', 'list', 'candidate_votes'], ['asc', 'asc', 'asc', 'desc']);
      ...
      const regularResults = Object.entries(groupedCandidates).map(([key, candidates]) => {
        const [city, circle, list] = key.split('-');
        const totalSeats = 7;
