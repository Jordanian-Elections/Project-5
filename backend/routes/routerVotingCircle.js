...
router.patch('/votedcircle', async (req, res) => {
    ...
    const { candidate_national_ids, circle_list, circle: candidateCircle, city: candidateCity } = candidate;
      ...
      if (userRecord.circle !== userCircle || userRecord.city !== userCity) {
        return res.status(400).json({ error: 'Provided user circle or city does not match the user\'s actual circle or city' });
      }
  
      if (userCircle !== candidateCircle || userCity !== candidateCity) {
        return res.status(400).json({ error: 'User\'s circle or city does not match the candidate\'s circle or city' });
      }
      ...
      if (candidate_national_ids && candidate_national_ids.length > 0) {
        ...
        const allSameCircleAndCity = candidatesDetails.every(c => c.circle === candidateCircle && c.city === candidateCity);
  
        if (!allSameCircleAndCity) {
          return res.status(400).json({ error: 'All candidates must be from the same circle and city as the user' });
        }
